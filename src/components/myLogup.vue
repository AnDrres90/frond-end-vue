<template>
    ...
    <div class="col-md-4 offset-md-4 container p-4">
        <form @submit.prevent="SaveUser()" class="card card-body form-group">
            ...
            <input type="password" placeholder="repit password" v-model="repitPassword" class="form-control mb-3">
            <button class="btn btn-primary">Send</button>
            <p class="text-center h6 m-3" style="color: #ea5959;">{{msg}}</p>
            <p class="p-3 text-center text-muted">I have an account <router-link to="/">Sign in</router-link>
            </p>
        </form>
    </div>
</template>
...
export default defineComponent({
    ...
    methods: {
        async SaveUser() {
            if(this.user.password === this.repitPassword){
                this.$router.push('/');
